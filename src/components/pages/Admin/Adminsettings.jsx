...
function AccountSection({ toast }) {
  const fileRef = useRef(null);
  const [avatar,  setAvatar]  = useState(null);
  const [form,    setForm]    = useState({ ...ACCOUNT_DEFAULTS });
  const [saved,   setSaved]   = useState({ ...ACCOUNT_DEFAULTS }); // last committed snapshot
...
function PasswordSection({ toast }) {
  ...
  const strengthColors = ["", "#e8302a", "#f0a500", "#f0a500", "#3ecf6e"];
...
function ThemeSection({ toast, onAccentChange }) {
  ...
  const [saved,  setSaved]  = useState({ ...DEFAULTS });
  const accents = ["#e86414","#3ecf6e","#4e6ef2","#f0a500","#a78bfa","#f87171","#06b6d4"];
...
export default function AdminSettings() {
  ...
  const renderSection = () => {
    switch (activeTab) {
      case "account":       return <AccountSection toast={showToast} />;
    ...
    <div className="as-root">
      ...
      <aside className="as-sidebar">
        <div className="as-sidebar__brand">
          <div className="as-sidebar__brand-icon"><Ic.Dumbbell /></div>
          <div className="as-sidebar__brand-name">FitTrack</div>
        </div>
        <div className="as-sidebar__label">Navigation</div>
        <ul className="as-sidebar__menu">
          {navItems.map(item => (
            <li key={item.route}
              className={isActive(item.route) ? "active" : ""}
              onClick={() => navigate(item.route)}>
              <span className="as-sidebar__icon">{navIcons[item.icon]}</span>
              {item.label}
              {isActive(item.route) && <span className="as-sidebar__dot" />}
            </li>
          ))}
        </ul>
        <div className="as-sidebar__bottom">
          <ul className="as-sidebar__menu">
            {/* Settings — marked active since we're on this page */}
            <li className={isActive("/admin/settings") ? "active" : ""}
              onClick={() => navigate("/admin/settings")}>
              <span className="as-sidebar__icon"><Ic.Settings /></span>Settings
              {isActive("/admin/settings") && <span className="as-sidebar__dot" />}
            </li>
            {/* ✅ Sidebar Logout — navigates to /login */}
            <li onClick={() => navigate("/login")}>
              <span className="as-sidebar__icon"><Ic.Logout /></span>Logout
            </li>
          </ul>
        </div>
      </aside>
      ...
      <div className="as-main">
        ...
        <div className="as-topbar">
          {/* ✅ Search — navigates to matching tab as you type */}
          <div className="as-topbar__search">
            <Ic.Search />
            <input
              ref={inputRef}
              type="text"
          ...
          <div className="as-topbar__right">
            <div className="as-topbar__notif"><Ic.Bell /><span className="as-topbar__notif-dot" /></div>
            <div className="as-topbar__profile">
              <div className="as-topbar__avatar">AD</div>
              <div>
                <div className="as-topbar__name">Admin Staff</div>
                <div className="as-topbar__role">FitTrack Admin</div>
              </div>
            </div>
          </div>
        </div>
