...
import GymLogin from "./Login";
import RegisterPage from "./Register";

function LandingPage() {

  // ── MODAL STATE ─────────────────────────────────────────────────
  // null = closed, "login" = show login, "register" = show register
  ...
  const handleFormSubmit = () => {
    ...
    setTimeout(() => {
      setSubmitting(false);
      showMessage("success", "✓ Message sent! We'll get back to you shortly.");
  ...
  return (
    <div className="app">
      ...
      {/* ── SINGLE PERSISTENT MODAL OVERLAY ─────────────────────
          The overlay stays mounted whenever activeModal is set.
          Only the inner content swaps — no unmount/remount flash.
      ──────────────────────────────────────────────────────────── */}
      {activeModal && (
        <div
        ...
        >
          <div className="login-modal-box">
            <div className="modal-content-switcher">
              {activeModal === "login" && (
                <GymLogin onClose={closeModal} onSwitchToRegister={openRegister} closeBtn={<button className="login-modal-close" onClick={closeModal} aria-label="Close"><FaTimes /></button>} />
              )}
              {activeModal === "register" && (
                <RegisterPage onClose={closeModal} onSwitchToLogin={openLogin} closeBtn={<button className="login-modal-close" onClick={closeModal} aria-label="Close"><FaTimes /></button>} />
              )}
            </div>
          </div>
        </div>
      ...
      <section id="pricing" className="pricing">
        ...
        <div className="pricing-grid">
          <PricingCard title="Basic" price="$49" features={["Membership Management","Attendance Tracking","Basic Reporting","Email Support"]} />
          <PricingCard popular title="Standard" price="$99" features={["All Basic features","Trainer Assignment","Online Payments","Performance Tracking","Priority Support"]} />
          <PricingCard title="Premium" price="$199" features={["All Standard features","Secure Access Integration","Custom Branding","Dedicated Account Manager"]} />
        </div>
      </section>
      ...
      <button className={`scroll-top-btn ${showTop ? "show" : ""}`} onClick={scrollToTop} aria-label="Scroll to top">↑</button>
    </div>
  );
}
