...
export default function MyMembership() {
  ...
  useEffect(() => {
    const fetchMembershipData = async () => {
      try {
        ...
        // Set default membership if not found
        const membershipPlan = memberData.MemberDetail?.membershipPlan || {
          name: "Standard Plan",
          tier: "Basic",
          daysRemaining: 0,
          startDate: new Date(),
        ...
        // Set default membership on error
        setMembership({
          name: "Loading failed",
          tier: "Unknown",
          daysRemaining: 0,
  ...
  const planName = membership?.name || "Standard Plan";
  const planTier = membership?.tier || "Basic";
  const startDate = membership?.startDate ? new Date(membership.startDate).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }) : "N/A";
  const endDate = membership?.endDate ? new Date(membership.endDate).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }) : "N/A";
      ...
      <main className="mym-main">
        ...
        <header className="mym-topbar">
          ...
          <div className="mym-topbar__right">
            <button className="mym-notif" onClick={() => showToast("No new notifications right now.")}>
              <IconBell />
              <span className="mym-notif__dot" />
            </button>
            <div className="mym-profile" onClick={() => navigate("/member/profile")}>
              <div className="mym-profile__avatar">{memberInitials}</div>
              <span className="mym-profile__name">{memberName}</span>
              <div className="mym-profile__globe">🌐</div>
            </div>
          </div>
        </header>
            ...
            <div className="mym-left">
              ...
              <div className="mym-plan-card">
                <div className="mym-plan-card__top">
                  <div>
                    <div className="mym-tier-pill">{planTier} Tier</div>
                    <div className="mym-plan-name">{planName}</div>
              ...
              <div className="mym-payment-card">
                ...
                <div className="mym-payment-card__amounts">
                  <div>
                    ...
                    <div className="mym-payment-card__amount">Rs 129.00</div>
                  </div>
