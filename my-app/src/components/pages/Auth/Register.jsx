...
export default function RegisterPage({ onClose, onSwitchToLogin, closeBtn }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMemberDisplayName(formData.fullName);
      ...
      const isDemoMode = Boolean(data?.demoMode);

      setAlert({
        type: "success",
        message: isDemoMode
          ? "✓ Account created in demo mode! Redirecting to login..."
              ...
              <div className="form-group">
                <div className="input-icon-wrap">
                  ...
                  <select name="plan" className={inputClass("plan")} value={formData.plan} onChange={handleChange} onBlur={handleBlur}>
                    ...
                    <option value="Monthly">Monthly — Rs 49/mo</option>
                    <option value="Quarterly">Quarterly — Rs 99/3mo</option>
                    <option value="Yearly">Yearly — Rs 199/yr</option>
                  </select>
                </div>
