...
const normalizeTrainerName = (value) =>
  String(value || "")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, " ")
    .trim();

const to24HourTime = (slotLabel) => {
  const safe = String(slotLabel || "").trim();
  const [time, period] = safe.split(" ");
  if (!time || !period) return "09:00:00";
  const [rawHour, rawMinute] = time.split(":");
  let hour = Number(rawHour);
  const minute = Number(rawMinute || 0);

  if (Number.isNaN(hour) || Number.isNaN(minute)) return "09:00:00";
...
function BookingModal({ trainer, onClose, onConfirm }) {
  ...
  const handleConfirm = async () => {
    ...
    setSubmitting(true);
    const success = await onConfirm({ trainer, date, timeSlot });
    setSubmitting(false);
        ...
        {confirmed ? (
          ...
          <>
            ...
            <div className="bt-modal__body">
              ...
              <div className="bt-modal__info">
                ...
                <div className="bt-modal__info-row">
                  ...
                  <span className="bt-modal__info-val bt-modal__info-val--brand">Rs {trainer.rate}.00</span>
                </div>
              </div>
            ...
            <div className="bt-modal__footer">
              ...
              <button className="bt-modal__confirm" onClick={handleConfirm} disabled={submitting}>
...
export default function BookTrainer() {
  ...
  const [trainerIdMap, setTrainerIdMap] = useState({});
  ...
  useEffect(() => {
    const fetchTrainers = async () => {
      ...
      try {
        ...
        const mapByName = {};
        const apiTrainers = Array.isArray(data.data) ? data.data : [];

        apiTrainers.forEach((trainer) => {
          const key = normalizeTrainerName(trainer?.name);
          if (key) {
            mapByName[key] = trainer.id;
          }
        });

        setTrainerIdMap(mapByName);
        setTrainers((prev) =>
          prev.map((trainer) => {
            const backendId = mapByName[normalizeTrainerName(trainer.name)] || null;
            return {
              ...trainer,
              backendId,
            };
        ...
        setToast(err.message || "Unable to load trainers from server.");
  ...
  const handleConfirm = async ({ trainer, date, timeSlot }) => {
    const token = localStorage.getItem("token");
    if (!token) {
    ...
    try {
      const backendId = trainer.backendId || trainerIdMap[normalizeTrainerName(trainer.name)];
      if (!backendId) {
        throw new Error("Trainer profile is not synced on server yet. Ask admin to add this trainer.");
      }

      const dateTime = `${date}T${to24HourTime(timeSlot)}`;
      const res = await fetch("http://localhost:5000/api/trainers/book", {
        method: "POST",
        headers: {
      ...
      setToast(`✓ Session requested with ${trainer.name} on ${new Date(date + "T00:00:00").toLocaleDateString("en-US",{month:"short",day:"numeric"})} at ${timeSlot}`);
      setTimeout(() => setToast(null), 4000);
      return true;
    } catch (err) {
      setToast(err.message || "Failed to submit booking request.");
  ...
  const handleNotificationClick = () => {
    setToast("No new notifications right now.");
    setTimeout(() => setToast(null), 2500);
  };

  const handleFilterReset = () => {
    setActiveTag("All Trainers");
    setSearch("");
      ...
      <main className="bt-main">
        ...
        <header className="bt-topbar">
          ...
          <div className="bt-topbar__right">
            <button className="bt-notif" onClick={handleNotificationClick}>
              <IconBell />
              <span className="bt-notif__dot" />
            </button>
            <div className="bt-profile" onClick={() => navigate("/member/profile")}>
              <div className="bt-profile__avatar">{memberInitials}</div>
              <span className="bt-profile__name">{memberName}</span>
        ...
        <div className="bt-content">
          ...
          <div className="bt-page-header">
            ...
            <div className="bt-header-controls">
              ...
              <button className="bt-filter-btn" onClick={handleFilterReset}>
                <IconFilter /> Filters
              </button>
            </div>
          </div>
